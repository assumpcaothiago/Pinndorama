...
from .._misc import default_verbose, tree_full_like, tree_where
...
class Zoom(AbstractSearch[Y, _FnInfo, FunctionInfo.EvalGrad, ZoomState]):
    ...
    When `verbose=True`, prints diagnostic information including:
        - Accepting stepsize, zoom iteration progress
        - Linesearch failure states and safe stepsize validity
        - Wolfe conditions satisfaction status
        - Interval bounds being zoomed into
    """
    ...
    verbose: bool | Callable[..., None] = default_verbose(False)
    ...
    def __post_init__(self):
        ...
        self.verbose = default_verbose(self.verbose)
    ...
    def _zoom_into_interval(self, y, y_eval, f_info, f_eval_info, state) -> ZoomState:
        ...
        # We're using pyright: ignore here because `verbose` is converted from
        # `bool | Callable[..., None]` to `Callable[..., None]` in `__post_init__`.
        self.verbose(  # pyright: ignore
            zoom_interval=(
                "Zooming into interval",
                (state.stepsize_lo, state.stepsize_hi),
            ),
            middle_stepsize=("Middle stepsize", stepsize_middle),
            decrease_satisfied=("Decrease satisfied", middle_satisf_decrease),
            curvature_satisfied=("Curvature satisfied", middle_satisf_curvature),
        )
        ...
        self.verbose(  # pyright: ignore
            interval_too_short=("Interval too short", interval_too_short),
            interval=("Interval", (new_stepsize_lo, new_stepsize_hi)),
        )
    ...
    def _search_interval(
        ...
        self.verbose(  # pyright: ignore
            searching_interval=("Searching for interval", True),
            checked_stepsize=("Checked stepsize", new_stepsize),
            decrease_satisfied=("Decrease satisfied", decrease_satisfied),
            curvature_satisfied=("Curvature satisfied", curvature_satisfied),
        )
    ...
    def _regular_step(
        ...
        """
        self.verbose(  # pyright: ignore
            zoom_iter=("Zoom iter", state.ls_iter_num),
        )
        ...
        self.verbose(  # pyright: ignore
            checked_stepsize=("Checked", state.stepsize),
    ...
    def step(
        ...
        self.verbose(  # pyright: ignore
            accepting=("Accepting stepsize", state.stepsize),
            accepted=("Accepted", accept & (~first_step)),
        )
