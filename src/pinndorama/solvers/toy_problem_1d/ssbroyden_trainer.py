"""Faithful shared-runtime SSBroyden refinement for the 1D loss."""
...
def run_ssbroyden_stage(
    ...
    *,
    ...
    kwargs = {
        ...
        "origin_regularity_weight": float(loss_weights["origin_regularity_weight"]),
        "outer_boundary_condition": str(loss_weights["outer_boundary_condition"]),
        "outer_boundary_weight": float(loss_weights["outer_boundary_weight"]),
    }
    start_time = time.time()
    history: list[float] = []
    initial_total, _ = loss.compute_loss(params, xx0, outer_xx0, **kwargs)
    best_state = {"params": _copy_tree(params), "loss": float(initial_total)}

    def objective_fn(current_params: Any):
        total, _ = loss.compute_loss(current_params, xx0, outer_xx0, **kwargs)
        return total
    ...
    def callback(
        ...
        if progress_callback is not None and (
            ...
            total, components = loss.compute_loss(
                current_params, xx0, outer_xx0, **kwargs
            )
            progress_callback(
                iteration,
                float(total),
    ...
    total, components = loss.compute_loss(best_params, xx0, outer_xx0, **kwargs)
    return (
        best_params,
        history,
        total,
