 ...
 * Step 4.a: Post-initial-data functions called after memory for non-y_n and auxevol gridfunctions is allocated.Step 5: MAIN SIMULATION LOOP
 ...
 * - Step 5.d: Step forward in time using Method of Lines with RK4 algorithm, applying outgoing radiation boundary conditions.
...
int main(int argc, const char *argv[]) {
  ...
  for (int grid = 0; grid < commondata.NUMGRIDS; grid++) {
    auxevol_gfs_set_to_constant(&commondata, &griddata[grid].params, griddata[grid].xx, &griddata[grid].gridfuncs);
#ifdef __CUDACC__
  ...
  // Write enriched 3D NRPyElliptic solution for external interpolation.
  write_NRPYELL_binary(&commondata, griddata);
