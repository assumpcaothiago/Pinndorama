...
#define NUM_PARAMETERS 29 // Define the number of parameters
 ...
 * nrpyelliptic_conformally_flat executable. It guides users on how to provide parameter
 * files and overwrite steerable parameters through command-line arguments.
 */
static void print_usage() {
  fprintf(stderr, "Usage option 0: ./nrpyelliptic_conformally_flat [--help or -h] - Outputs this usage command\n");
  fprintf(stderr, "Usage option 1: ./nrpyelliptic_conformally_flat - Reads in parameter file nrpyelliptic_conformally_flat.par\n");
  fprintf(stderr, "Usage option 2: ./nrpyelliptic_conformally_flat [parfile] - Reads in parameter file [parfile]\n");
  fprintf(stderr, "Usage option 3: ./nrpyelliptic_conformally_flat [convergence_factor] - Overwrites parameters in list after reading in "
                  "nrpyelliptic_conformally_flat.par\n");
  fprintf(stderr, "Usage option 4: ./nrpyelliptic_conformally_flat [parfile] [convergence_factor] - Overwrites list of steerable parameters after "
                  "reading in [parfile]\n");
}
...
param_descriptor param_table[] = {{"CFL_FACTOR", 0, PARAM_REAL, 0, 0},
                                  ...
                                  {"P0_x", 2, PARAM_REAL, 0, 0},
                                  {"P0_y", 3, PARAM_REAL, 0, 0},
                                  {"P0_z", 4, PARAM_REAL, 0, 0},
                                  {"P1_x", 5, PARAM_REAL, 0, 0},
                                  {"P1_y", 6, PARAM_REAL, 0, 0},
                                  {"P1_z", 7, PARAM_REAL, 0, 0},
                                  {"S0_x", 8, PARAM_REAL, 0, 0},
                                  {"S0_y", 9, PARAM_REAL, 0, 0},
                                  {"S0_z", 10, PARAM_REAL, 0, 0},
                                  {"S1_x", 11, PARAM_REAL, 0, 0},
                                  {"S1_y", 12, PARAM_REAL, 0, 0},
                                  {"S1_z", 13, PARAM_REAL, 0, 0},
                                  {"bare_mass_0", 14, PARAM_REAL, 0, 0},
                                  {"bare_mass_1", 15, PARAM_REAL, 0, 0},
                                  {"checkpoint_every", 16, PARAM_REAL, 0, 0},
                                  {"convergence_factor", 17, PARAM_REAL, 0, 0},
                                  {"diagnostics_output_every", 18, PARAM_REAL, 0, 0},
                                  {"eta_damping", 19, PARAM_REAL, 0, 0},
                                  {"log10_current_residual", 20, PARAM_REAL, 0, 0},
                                  {"log10_residual_tolerance", 21, PARAM_REAL, 0, 0},
                                  {"nn_max", 22, PARAM_INT, 0, 0},
                                  {"outer_bc_type", 23, PARAM_CHARARRAY, 0, 50},
                                  {"output_progress_every", 24, PARAM_INT, 0, 0},
                                  {"stop_relaxation", 25, PARAM_BOOL, 0, 0},
                                  {"t_final", 26, PARAM_REAL, 0, 0},
                                  {"zPunc", 27, PARAM_REAL, 0, 0},
                                  {"initial_guess", 28, PARAM_CHARARRAY, 0, 32}};
...
void cmdline_input_and_parfile_parser(commondata_struct *restrict commondata, int argc, const char *argv[]) {
  ...
  if (argc == 1) {
    option = 1; // Usage option 1: Process default parameter file "nrpyelliptic_conformally_flat.par"
  } else if (argc == 2) {
    ...
    option = 3; // Usage option 3: Overwrite steerable parameters after processing "nrpyelliptic_conformally_flat.par"
  ...
  const char *filename = (option == 1 || option == 3) ? "nrpyelliptic_conformally_flat.par" : argv[1];
    ...
    if (p) {
      ...
      if (p) {
        ...
        if (param_desc->index == 0) {
          ...
          read_REAL(values_array[0], &commondata->checkpoint_every, "checkpoint_every");
          ...
          read_chararray(values_array[0], commondata->initial_guess, "initial_guess", 32);
        } else {
