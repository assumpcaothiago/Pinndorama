...
void read_NRPYELL_binary(const char *restrict binary_path,
  ...
  FILE *restrict fp = fopen(binary_path, "rb");
  if (fp == NULL) {
    fprintf(stderr, "NRPYELL: Could not open binary '%s': %s\n", binary_path, strerror(errno));
    exit(1);
  }
  ...
  if (memcmp(magic, NRPYELL_BINARY_MAGIC, NRPYELL_BINARY_MAGIC_LEN) != 0) {
    fprintf(stderr, "NRPYELL: Unsupported binary magic. Expected 3D NRPYELL_solution.bin.\n");
    fclose(fp);
    exit(1);
  }
  ...
  *NRPYELL_fields = (REAL *restrict)malloc(total_fields * sizeof(REAL));
  ...
  fclose(fp);
  printf("NRPYELL: FINISHED READING 3D '%s'\n", binary_path);
