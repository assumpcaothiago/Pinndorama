...
enum { INTERP_SUCCESS, INTERP2D_GENERAL_NULL_PTRS, INTERP2D_GENERAL_INTERP_ORDER_GT_NXX123, INTERP2D_GENERAL_HORIZON_OUT_OF_BOUNDS } error_codes;

#pragma GCC optimize("unroll-loops")

/**
 * Performs 2D Lagrange interpolation (specialized from the 3D version) from a set of uniform grid points on the source grid to arbitrary destination
 * points. The interpolation is performed in the x0 and x1 directions only; the x2 coordinate is fixed.
 *
...
#pragma omp parallel for
  for (int dst_pt = 0; dst_pt < num_dst_pts; dst_pt++) {
    ...
    if ((idx_center_x0 - NinterpGHOSTS < 0) || (idx_center_x0 + NinterpGHOSTS >= src_Nxx_plus_2NGHOSTS0) || (idx_center_x1 - NinterpGHOSTS < 0) ||
        (idx_center_x1 + NinterpGHOSTS >= src_Nxx_plus_2NGHOSTS1)) {
#pragma omp critical
      {
...
#pragma omp simd
  ...
  return error_flag;
} // END FUNCTION interpolation_2d_general__uniform_src_grid
