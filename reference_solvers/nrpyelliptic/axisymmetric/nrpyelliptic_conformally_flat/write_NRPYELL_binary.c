...
/**
 * Writes enriched NRPyElliptic data to NRPYELL_solution.bin.
 *
 * Binary v2 layout:
 * 1. Eight-byte magic string NRPYELL2.
 ...
 * 5. NRPYELL_SOL_NUM_GFS field arrays, each of size total points, ordered as in
 *    NRPYELL_binary_format.h.
 */
...
#include "set_CodeParameters.h"
  ...
  if (NRPYELL_fields == NULL) {
    perror("NRPYELL: Memory allocation failed for enriched solution fields");
    exit(1);
  }
...
#pragma omp parallel for collapse(2)
  ...
  compute_residual_all_points(commondata, params, rfmstruct, auxevol_gfs, y_n_gfs, &NRPYELL_fields[IDX4pt(NRPYELL_SOL_RESIDUAL_HGF, 0)]);
  NRPYELL_derivatives_all_points(commondata, params, y_n_gfs, NRPYELL_fields);
  ...
  fclose(fp);
  free(NRPYELL_fields);
  printf("NRPYELL: FINISHED WRITING enriched 'NRPYELL_solution.bin' with %d fields\n", NRPYELL_NUM_FIELDS);
} // END FUNCTION write_NRPYELL_binary
